...
# def integrand_y(y):
#     return find_integral(-np.sqrt(4 - y ** 2), np.sqrt(4 - y**2), N_X, lambda x: integrand(x, y))

# res = sympson(np.linspace(-2, 2, N_Y), integrand_y)
# print(f"Двукратный интеграл: {res}")

def integ(x):
    return find_integral(-np.sqrt(4 - x ** 2), np.sqrt(4 - x ** 2), N_Y, lambda y: x ** 2 * y ** 2)

res = sympson(np.linspace(-2, 2, N_X), integ)
print(f"Двукратный интеграл: {res}")

# f = Spline(data)
# integ = lambda y: find_integral(0, 1 - y, N_X, lambda x: f(x, y))

# res = sympson(np.linspace(0, 1, N_Y), integ)
# print(f"Двукратный интеграл: {res}")

# z = [[np.log(data[2][j][i]) for i in range(len(data[0]))] for j in range(len(data[1]))]
# data[2] = z
# integ = lambda y: find_integral(0, 1 - y, N_X, lambda x: np.exp(f(x, y)))
